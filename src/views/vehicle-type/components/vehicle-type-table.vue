...
import type { VehicleTypeInput } from '@/apis/__generated/model/static'
import { Delete, Edit, Plus } from '@element-plus/icons-vue'
import DictColumn from '@/components/dict/dict-column.vue'
import { DictConstants } from '@/apis/__generated/model/enums/DictConstants'
type VehicleTypeScope = Scope<VehicleTypeDto['VehicleTypeRepository/COMPLEX_FETCHER']>
...
const handleCreate = () => {
  openDialog('CREATE')
  createForm.value = { ...vehicleTypeStore.initForm }
}
...
const handleDelete = (ids: string[]) => {
  ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
    ...
    api.vehicleTypeController.delete({ body: ids }).then((res) => {
...
<template>
  <div>
    ...
    >
      ...
      <el-table-column label="型号" prop="typeName" sortable="custom">
        <template v-slot:default="{ row }: VehicleTypeScope">
          {{ row.typeName }}
        </template>
      </el-table-column>
      <el-table-column label="型号大小" prop="typeSize" sortable="custom">
        <template v-slot:default="{ row }: VehicleTypeScope">
          {{ row.typeSize }}
        </template>
      </el-table-column>
      <el-table-column label="燃油类型" prop="fuelType" sortable="custom">
        <template v-slot:default="{ row }: VehicleTypeScope">
          {{ row.fuelType }}
        </template>
      </el-table-column>
      <el-table-column label="制造厂家" prop="manufacturer" sortable="custom">
        <template v-slot:default="{ row }: VehicleTypeScope">
          {{ row.manufacturer }}
        </template>
      </el-table-column>
      <el-table-column label="正座数量" prop="passengerCapacity" sortable="custom">
        <template v-slot:default="{ row }: VehicleTypeScope">
          {{ row.passengerCapacity }}
        </template>
      </el-table-column>
      <el-table-column label="副座数量" prop="extraPassengerCapacity" sortable="custom">
        <template v-slot:default="{ row }: VehicleTypeScope">
          {{ row.extraPassengerCapacity }}
        </template>
      </el-table-column>
      <el-table-column label="行李容量" prop="luggageCapacity" sortable="custom">
        <template v-slot:default="{ row }: VehicleTypeScope">
          {{ row.luggageCapacity }}
        </template>
      </el-table-column>
